...
def recognize_and_submit_captcha(driver, ticket_num, max_attempts=5):
...
def main():
    ...
    # 等開賣並點「立即購票」
    print("刷新中等待開賣…")
    btn = wait_for_sale_start(driver)
    driver.execute_script("arguments[0].click()", btn)
    # 選場次
    ...
    #辨識和提交驗證碼
    final_code = recognize_and_submit_captcha(driver, ticket_num)
    print(f"\n最終驗證碼：{final_code}")
